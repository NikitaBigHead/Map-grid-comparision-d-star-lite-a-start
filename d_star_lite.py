...
# d_star_lite.py
"""
Implementation of D* Lite algorithm for path planning with a 5x5 robot.
Supports incremental replanning when the map changes (e.g., new obstacles discovered).
"""
...
class DStarLite:
    def __init__(self, grid: np.ndarray, robot_size: int = 5):
        """
        Initialize D* Lite with the current known map.
        
        Args:
            grid: Current known map (1 = obstacle, 0 = free or unknown).
            robot_size: Size of the robot (default 5 for 5x5 footprint).
        """
        self.grid = grid.copy()
        self.height, self.width = grid.shape
        self.robot_size = robot_size
    ...
    def reset(self):
        """Reset internal state for a new planning problem."""
        self.start = None
        self.goal = None
        self.last_start = None
        self.km = 0.0

        self.g = defaultdict(lambda: float('inf'))    # actual cost from start
    ...
    def calculate_key(self, s: Tuple[int, int]) -> Tuple[float, float]:
        """Calculate the priority key for node s."""
        min_val = min(self.g[s], self.rhs[s])
    ...
    def is_position_valid(self, pos: Tuple[int, int]) -> bool:
        """Check if the 5x5 robot can be centered at pos."""
        x, y = pos
    ...
    def get_neighbors(self, pos: Tuple[int, int]) -> List[Tuple[Tuple[int, int], float]]:
        """Return valid neighbors with movement cost (used as predecessors in reversed graph)."""
        x, y = pos
        neighbors = []
        for i, (dx, dy) in enumerate(self.directions):
            ...
            if self.is_position_valid(neighbor):
                neighbors.append((neighbor, self.costs[i]))
        return neighbors

    def update_vertex(self, u: Tuple[int, int]):
        """Update rhs value and insert into queue if inconsistent."""
        if u != self.goal:
            self.rhs[u] = min((cost + self.g[v] for v, cost in self.get_neighbors(u)),
                              default=float('inf'))
    ...
    def compute_shortest_path(self):
        """Main search loop with correct D* Lite termination condition."""
        self.visited_count = 0
        while self.U:
            current_key, u = heapq.heappop(self.U)
            self.visited_count += 1

            # Skip outdated entries
            if current_key > self.calculate_key(u):
                continue
            ...
            # Update predecessors and self
            for pred, _ in self.get_neighbors(u):
                self.update_vertex(pred)
            self.update_vertex(u)

            # Stop when start is locally consistent and has the best key
            if (self.top_key() >= self.calculate_key(self.start) and
                self.rhs[self.start] <= self.g[self.start]):
                break

    def initialize(self, start: Tuple[int, int], goal: Tuple[int, int]):
        """Initialize for a new planning problem."""
        self.reset()
        self.start = start
    ...
    def move_to(self, new_start: Tuple[int, int]):
        """Update when the robot moves to a new position."""
    ...
    def update_obstacles(self, changed_cells: List[Tuple[int, int]]):
        """Handle map changes (e.g., newly discovered obstacles in visibility range)."""
        affected = set(changed_cells)
        for cell in changed_cells:
            for neigh, _ in self.get_neighbors(cell):
                affected.add(neigh)
        for u in affected:
            self.update_vertex(u)
        self.compute_shortest_path()
    ...
    def extract_path(self) -> List[Tuple[int, int]]:
        ...
        for _ in range(max_steps):
            ...
            if not neighbors:
                break
            # Choose neighbor with minimal g-value + movement cost
            current = min(neighbors, key=lambda nc: self.g[nc[0]] + nc[1])[0]
            path.append(current)
    ...
    def get_path_info(self) -> Optional[Dict]:
        """Return path and statistics if a valid path exists."""
        path = self.extract_path()
        if not path or self.g[self.start] == float('inf'):
            return None
        return {
            'path': path,
            'visited_count': self.visited_count,
            'path_length': len(path) - 1,
            'total_cost': self.g[self.start]
        }